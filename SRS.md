...
Il software è commissionato dall'azienda **AS GAIA** (www.asgaia.it) per attività di team building e turismo esperienziale.

Questo SRS è redatto secondo lo standard **IEEE 830-1998** e i criteri di qualità di Sommerville (correttezza, completezza, non ambiguità, consistenza, verificabilità, modificabilità, tracciabilità). Serve come riferimento per tutte le fasi del ciclo di vita: progettazione, implementazione, testing e manutenzione.
...
|------------|-------------|------------|
| **PWA Giocatore** | Progressive Web App per dispositivi mobili. Consente navigazione GPS, sblocco domande via geofence, risposta a tempo, visualizzazione classifica in tempo reale, recupero GPS. | Next.js, React, Leaflet.js, Tailwind CSS/DaisyUI, Zustand |
...
**Fuori dall'ambito:**
- Applicazioni native iOS/Android (la PWA è sufficiente)
- Integrazione con social network
- Sistema di pagamento
- Realtà aumentata (AR)
- Persistenza dati oltre la durata della partita
...
Il sistema è un'applicazione standalone che interagisce con servizi esterni per mappe, generazione di contenuti e persistenza temporanea. L'architettura è **3-tier**: presentazione (PWA), logica applicativa (Edge Functions), dati (PostgreSQL con dati effimeri).
...
| **Operatore** | La persona che gestisce l'escape room il giorno dell'evento. Posiziona i POI sulla mappa, sceglie il tema delle domande, crea le squadre, avvia e ferma la partita, monitora l'andamento. Non richiede competenze tecniche avanzate. | Familiarita base con interfacce web e mappe. | Pannello Operatore desktop |
